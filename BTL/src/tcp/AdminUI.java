...
public class AdminUI extends JFrame {
    ...
    // --- THAY ĐỔI: Nhận tên admin từ constructor ---
    public AdminUI(String username) {
        ...
        // --- THAY ĐỔI: Sửa cấu trúc actionPanel để thêm thông tin admin ---
        JPanel actionPanel = new JPanel(new BorderLayout()); // Dùng BorderLayout
        actionPanel.setOpaque(false);
        actionPanel.setBorder(new EmptyBorder(0, 0, 0, 10));

        // Panel cho các nút chức năng chính
        ...
        topButtonsPanel.add(Box.createRigidArea(new Dimension(0, 15)));
        topButtonsPanel.add(btnEdit);
        topButtonsPanel.add(Box.createRigidArea(new Dimension(0, 15)));
        topButtonsPanel.add(btnDelete);
        topButtonsPanel.add(Box.createRigidArea(new Dimension(0, 15)));
        topButtonsPanel.add(btnViewHistory);

        actionPanel.add(topButtonsPanel, BorderLayout.NORTH);

        // Panel cho thông tin admin (avatar và tên)
        ...
        try {
            ImageIcon adminIcon = new ImageIcon(getClass().getResource("/images/teacher.png"));
            avatarLabel.setIcon(createCircularIcon(adminIcon, 80));
        } catch (Exception e) {
            System.err.println("Không tìm thấy ảnh avatar admin: /images/teacher.png");
        }

        JLabel usernameLabel = new JLabel(this.username);
        usernameLabel.setFont(Theme.getBoldFont(16f));
        usernameLabel.setForeground(Theme.TEXT_DARK); // Màu tối cho dễ đọc trên nền trắng
        ...
        // Panel bọc để căn giữa khu vực thông tin admin
        JPanel profileWrapper = new JPanel(new GridBagLayout());
        profileWrapper.setOpaque(false);
        profileWrapper.add(userProfilePanel);
        actionPanel.add(profileWrapper, BorderLayout.CENTER);
        ...
        actionPanel.add(btnLogout, BorderLayout.SOUTH);
    ...
    // --- THÊM MỚI: Phương thức tạo icon tròn (copy từ DoExamUI) ---
